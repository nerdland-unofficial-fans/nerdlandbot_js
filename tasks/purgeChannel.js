...
const startPurgeChannelTasks = async (client) => {
  _client = client
  const guildsData = await getAllGuilds()
  guildsData.forEach(guildData => {
    Object.values(guildData.purgers).forEach(purger => {
      addPurgerAndStartTask(purger)
    })
  })
}

const stopPurgeChannelTasks = () => {
  tasks.forEach(task => task.task.stop())
}

const stopTask = async channelId => {
  const foundTask = tasks.find(task => task.channelId === channelId)
  if (!foundTask) {
    return false
  }
  foundTask.task.stop()
  log.info(`stopped Purge task on channel <#${channelId}>`)
...
const addPurgerAndStartTask = (purger) => {
  ...
  tasks.push({ channelId: purger.channelId, task })
  log.debug(`added new Purge task to channel <#${purger.channelId}>: "${purger.description}" - max ${purger.maxAge} uur - cronTime: ${purger.cronTime}`)
}

module.exports = { startPurgeChannelTasks, stopPurgeChannelTasks, addPurgerAndStartTask, stopTask }
